...
- 입력값:
  ...
  - **Bundle ID: `uk.terrylovesapp.lovetoday`** — `frontend/app.json`의 `ios.bundleIdentifier`와 **글자까지 정확히 일치**해야 합니다. (Bundle ID 목록에 없으면 먼저 Certificates, Identifiers & Profiles에서 등록하거나, 아래 1.4 `eas credentials`가 자동 생성하도록 둡니다.)
